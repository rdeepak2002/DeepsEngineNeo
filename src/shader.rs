...
fn get_glsl_version() -> &'static str {
...
impl Shader {
    pub fn new(vertexPath: &str, fragmentPath: &str) -> Shader {
        ...
        let vsc = format!("{}\n{}", get_glsl_version(), vertexCode);
        let vShaderCode = CString::new(vsc.as_str().as_bytes()).unwrap();

        let fsc = format!("{}\n{}", get_glsl_version(), fragmentCode);
        let fShaderCode = CString::new(fsc.as_str().as_bytes()).unwrap();
