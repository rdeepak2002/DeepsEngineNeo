...
# build
cd ..
(rm -rf ./build-scripts/web-dist) && (EMCC_CFLAGS="--use-preload-plugins -s WASM=1 -s ASSERTIONS -s FORCE_FILESYSTEM=1 -s LLD_REPORT_UNDEFINED -s ALLOW_MEMORY_GROWTH=1 -s FULL_ES3=1 -s USE_WEBGL2=1 -s MIN_WEBGL_VERSION=2 -s MAX_WEBGL_VERSION=2 -s USE_SDL=2 --preload-file examples" CARGO_TARGET_DIR=./build-scripts/web-dist cargo build --"${BUILD_TYPE}" --target wasm32-unknown-emscripten --verbose)
...
then
  ...
  cp ./index.html ./web-dist/wasm32-unknown-emscripten/${BUILD_TYPE}/deps/index.html
  cd ./web-dist/wasm32-unknown-emscripten/${BUILD_TYPE}/deps || exit 1
